...
export default function MovingTrillsListScreen() {
  ...
  const [selectedChart, setSelectedChart] = useState<MovingTrillChartItem | null>(
    null
  );
  const [selectedDifficulty, setSelectedDifficulty] =
    useState<DifficultyMovingTrillAnalysis | null>(null);
  ...
  const fetchCharts = async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      const response = await ChartAPI.getMovingTrillCharts({
        sortBy,
        order,
        page: pageNum,
        limit: 30,
      ...
      console.error("Error fetching moving trill charts:", err);
  ...
  const handleChartPress = (chart: MovingTrillChartItem) => {
  ...
  const renderChartItem = ({ item }: { item: MovingTrillChartItem }) => {
        ...
        <View style={styles.chartInfo}>
          ...
          <View style={styles.statsRow}>
            <View style={styles.statItem}>
              <Ionicons name="musical-notes" size={14} color="#FF6B35" />
  ...
  const renderContent = () => {
    ...
    if (charts.length === 0) {
      return (
        <ThemedView style={styles.centerContainer}>
          <Ionicons name="shuffle" size={60} color="#CCCCCC" />
          <ThemedText style={styles.emptyText}>
            {t("noMovingTrillChartsFound")}
          </ThemedText>
        </ThemedView>
      );
    }
          ...
          >
            ...
            <ScrollView style={styles.trillDetailsContainer}>
              <View style={styles.trillSectionHeader}>
                ...
                >
                  {t("movingTrillDetails")} ({selectedDifficulty.trillCount}{" "}
                  {t("movingTrillsFound")})
                </ThemedText>
                ...
                >
                  ...
                  <View style={styles.trillStats}>
                    ...
                    <View style={styles.trillStatRow}>
                      ...
                      <ThemedText style={styles.trillStatValue} numberOfLines={10}>
