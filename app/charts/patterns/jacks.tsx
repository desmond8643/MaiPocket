...
import { DifficultyJackAnalysis, JackChartItem } from "@/types/chart";
...
type SortOption = "longestJack" | "speed" | "level" | "jackCount";
type OrderOption = "asc" | "desc";

const SORT_OPTIONS: { key: SortOption; labelKey: string }[] = [
  { key: "longestJack", labelKey: "sortLongestJack" },
  { key: "speed", labelKey: "sortSpeed" },
  { key: "level", labelKey: "sortLevel" },
  { key: "jackCount", labelKey: "sortJackCount" },
];
...
export default function JacksListScreen() {
  ...
  const [charts, setCharts] = useState<JackChartItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState<SortOption>("longestJack");
  ...
  const [selectedChart, setSelectedChart] = useState<JackChartItem | null>(
    null
  );
  const [selectedDifficulty, setSelectedDifficulty] =
    useState<DifficultyJackAnalysis | null>(null);
  ...
  const fetchCharts = async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      const response = await ChartAPI.getJackCharts({
        sortBy,
        order,
        page: pageNum,
        limit: 30,
      ...
      console.error("Error fetching jack charts:", err);
  ...
  const handleChartPress = (chart: JackChartItem) => {
  ...
  const renderChartItem = ({ item }: { item: JackChartItem }) => {
    // Find the longest/fastest jack difficulty for display
    const primaryDiff = item.difficulties.reduce((best, curr) => {
      if (sortBy === "speed") {
        return curr.fastestSpeed > best.fastestSpeed ? curr : best;
      }
      return curr.longestJack > best.longestJack ? curr : best;
    }, item.difficulties[0]);
        ...
        <View style={styles.chartInfo}>
          ...
          <View style={styles.statsRow}>
            <View style={styles.statItem}>
              <Ionicons name="radio-button-on" size={14} color="#9C27B0" />
              <ThemedText style={styles.statText}>
                {item.maxJackLength} {t("notes")}
              </ThemedText>
            </View>
            <View style={styles.statItem}>
              <Ionicons name="speedometer" size={14} color="#9C27B0" />
              <ThemedText style={styles.statText}>
                BPM {item.fastestBpm} {`{${item.fastestDivision}}`} ({item.maxSpeed.toFixed(1)}/s)
              </ThemedText>
            </View>
          </View>
        </View>
  ...
  const renderContent = () => {
    ...
    if (charts.length === 0) {
      return (
        <ThemedView style={styles.centerContainer}>
          <Ionicons name="radio-button-on" size={60} color="#CCCCCC" />
      ...
      >
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            {/* Jack details */}
            <ScrollView style={styles.jackDetailsContainer}>
              <View style={styles.jackSectionHeader}>
                <ThemedText
                  type="defaultSemiBold"
                  style={styles.jackSectionTitle}
                >
                  {t("jackDetails")} ({selectedDifficulty.jackCount}{" "}
                  {t("jacksFound")})
                </ThemedText>
              ...
              {selectedDifficulty.jacks.map((jack, idx) => (
                <View
                  key={idx}
                  style={[
                    styles.jackItem,
                ...
                >
                  <View style={styles.jackItemHeader}>
                    <ThemedText
                      type="defaultSemiBold"
                      style={styles.jackItemTitle}
                    >
                      {t("jack")} #{idx + 1}
                    </ThemedText>
                    <ThemedText style={styles.jackTimestamp}>
                      ⏱️ {jack.timestamp}
                    </ThemedText>
                  </View>

                  <View style={styles.jackStats}>
                    <View style={styles.jackStatRow}>
                      <Ionicons name="radio-button-on" size={16} color="#9C27B0" />
                      <ThemedText style={styles.jackStatLabel}>
                        {t("length")}:
                      </ThemedText>
                      <ThemedText style={styles.jackStatValue}>
                        {jack.length} {t("notes")}
                      </ThemedText>
                    </View>

                    <View style={styles.jackStatRow}>
                      <Ionicons name="locate" size={16} color="#9C27B0" />
                      <ThemedText style={styles.jackStatLabel}>
                        {t("button")}:
                      </ThemedText>
                      <ThemedText style={styles.jackStatValue}>
                    ...
                    <View style={styles.jackStatRow}>
                      ...
                      <ThemedText style={styles.jackStatValue}>
                        BPM {jack.bpm} {`{${jack.effectiveDivision}}`}
                      </ThemedText>
                    </View>
                  </View>
                </View>
          ...
          headerRight: () => (
            ...
            >
              <Ionicons
                name="filter"
                size={16}
                color={levelFilter !== "All" ? "white" : "#9C27B0"}
              />
              <ThemedText
                style={[
